...
function artifactReportItem(artifact, index) {
  return {
    id: `report-${artifact.id}`,
    type: artifact.type,
    title: artifact.title || `Workbench artifact ${index + 1}`,
    detail: artifact.content ? artifact.content.slice(0, 240) : "AI Workbench artifact saved from the workspace.",
    status: artifactStatus(artifact.status),
    period: "AI Workbench",
    owner: artifact.owner?.position || artifact.owner?.role || "GENIUS",
    formats: ["json", "csv", "md"],
    summary: [
      artifact.content || "AI Workbench artifact saved from the workspace.",
      artifact.metrics?.expectedSavings ? `${money(artifact.metrics.expectedSavings)} target savings.` : "",
      artifact.metrics?.riskExposure ? `${money(artifact.metrics.riskExposure)} risk exposure in scope.` : "",
      artifact.metrics?.confidence ? `${artifact.metrics.confidence}% confidence.` : "",
    ].filter(Boolean),
    metrics: {
      moneyAtRisk: artifact.metrics?.riskExposure || 0,
      expectedSavings: artifact.metrics?.expectedSavings || 0,
      confidence: artifact.metrics?.confidence || 0,
      findings: artifact.recommendations?.length || 0,
      openApprovals: 0,
    },
    sections: [
      {
        ...
        items: [
          {
            label: artifact.title || "Workbench artifact",
            value: artifactStatus(artifact.status),
            evidence: artifact.content || "Saved from AI Workbench.",
      ...
      },
      {
        ...
        items: (artifact.recommendations || []).map((item) => ({
          label: item.title,
          value: item.impact || "Review",
          evidence: item.detail || "Workbench recommendation",
          artifactId: artifact.id,
        })),
      },
    ],
...
export function buildWorkspaceReports(workspace) {
  ...
  const artifactReports = (workspace.workspaceArtifacts || [])
    .filter((artifact) => artifact.type === "executive_summary" && artifact.status !== "archived")
    .slice(0, 10)
    .map(artifactReportItem);
  ...
  if (!hasReportInput && !artifactReports.length) return [];
  ...
  if (format === "csv") {
    const rows = [
      ...
      [
        "Report",
        "Title",
        detail.report.title || detail.report.id || "",
