...
export async function deleteEvidenceFiles(records = [], options = {}) {
  if (!isSupabaseFileStorageConfigured()) return;

  const workspaceId = String(options.workspaceId || "").trim();
  const requiredPrefix = workspaceId ? `${workspaceId}/` : "";
  const files = records
    .map((record) => record?.storage || record?.extracted?.storage_file)
    .filter((file) => {
      const objectPath = String(file?.path || "");
      return file?.bucket === evidenceBucket
        && objectPath
        && (!requiredPrefix || objectPath.startsWith(requiredPrefix));
    });
