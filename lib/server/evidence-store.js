...
// Keeps MVP data local to the project until we replace it with a real database.
function getDataDirectory() {
  return path.join(process.cwd(), ".data");
}

// Single JSON file is enough for pre-customer MVP testing and easy inspection.
function getWorkspacePath() {
  return path.join(getDataDirectory(), workspaceFileName);
}

// Defines the stable workspace shape so missing files and old files load safely.
...
// Writes the full workspace atomically enough for a single-user local MVP.
async function writeWorkspace(workspace) {
  await ensureDataDirectory();
  const syncedWorkspace = syncDerivedWorkspaceData(normalizeWorkspace(workspace));
  const nextWorkspace = {
    ...syncedWorkspace,
    ...workspace,
    findings: syncedWorkspace.findings,
    actions: syncedWorkspace.actions,
    agentRuns: syncedWorkspace.agentRuns,
    notifications: syncedWorkspace.notifications,
    connectors: syncedWorkspace.connectors,
    metrics: syncedWorkspace.metrics,
    updatedAt: new Date().toISOString(),
  };
  await writeFile(getWorkspacePath(), `${JSON.stringify(nextWorkspace, null, 2)}\n`, "utf8");
