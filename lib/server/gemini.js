...
export async function streamChatWithGemini({ messages, evidence, liveEvents, diagnostics, proofTrail, findings, actions, reports, language }) {
  const { apiKey, model } = getGeminiConfig();
  ...
  const response = await fetch(`${geminiEndpoint}/${model}:streamGenerateContent?alt=sse`, {
    ...
    body: JSON.stringify({
      systemInstruction: {
        parts: [{ text: buildChatSystemPrompt({ evidence, liveEvents, diagnostics, proofTrail, findings, actions, reports, language }) }],
      },
      contents,
      generationConfig: {
        temperature: 0.35,
